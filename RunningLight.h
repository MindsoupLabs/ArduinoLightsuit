...
class RunningLight : public ChainableLight {
	public:
		RunningLight(unsigned int startPosition, unsigned int ledsPerSecondSpeed, unsigned int size, unsigned int lifetime, ColorGenerator* colorGenerator, ChainableLightListener* listener);
		virtual ~RunningLight();
		virtual void loop(VolumeContext &context);

	private:
		unsigned int startPosition;
		unsigned int ledsPerSecondSpeed;
		unsigned int size;
